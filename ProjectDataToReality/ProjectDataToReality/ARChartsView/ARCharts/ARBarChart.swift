...
class ARBarChart: SCNNode {
    
    var dataSource: ARBarChartDataSource?
    var delegate: ARBarChartDelegate?
    var size: SCNVector3!
    var animationType: ARChartPresenter.AnimationType? { didSet { updatePresenter() } }
    var animationDuration = 1.0 { didSet { updatePresenter() } }
    
    private var presenter: ARChartPresenter!
    private var highlighter: ARChartHighlighter!

    // for highlight label
    private var highlightLabelNodePositions = [[SCNVector3]]()
    private var highlightLabelNode: ARBarchartHighlightLabel?
    ...
    func draw() {
        ...
        highlightLabelNodePositions.removeAll()
        for series in 0..<numberOfSeries {
            ...
            var seriesBarPositions = [SCNVector3]()
            ...
            for index in 0..<dataSource.barChart(self, numberOfValuesInSeries: series) {
                ...
                seriesBarPositions.append(SCNVector3(xPosition + xShift, barHeight, zPosition + zShift))

                //                let highlightLabelNode = delegate.barChart(self, labelNodeAtIndex: index, atSeries: series)
                //                highlightLabelNode.position = SCNVector3(barWidth / 2, barHeight / 1.5, barLength / 2)
                //                barNode.addChildNode(highlightLabelNode)
            ...
            highlightLabelNodePositions.append(seriesBarPositions)
     ...
     */
    func highlightBar(atIndex index: Int,
        ...
        if let hlLabelNode = delegate?.barChart(self, labelNodeAtIndex: index, atSeries: series) {
            hlLabelNode.position = highlightLabelNodePositions[series][index]
            self.addChildNode(hlLabelNode)
            highlightLabelNode = hlLabelNode
        }
    ...
    func unhighlight() {
        ...
        highlightLabelNode?.removeFromParentNode()
    }
